...
const RISK_LEVELS = [
  { key: 'Nguy Hiểm', icon: XCircle,      color: 'text-red-600',    bg: 'bg-red-600',    border: 'border-red-500',    light: 'bg-red-50 border-red-200',    desc: 'Cần gặp bác sĩ chuyên khoa ngay lập tức. Không được trì hoãn.' },
  { key: 'Cao',       icon: AlertTriangle, color: 'text-orange-600', bg: 'bg-orange-500', border: 'border-orange-400',  light: 'bg-orange-50 border-orange-200', desc: 'Cần khám da liễu khẩn cấp trong vòng 2–4 tuần.' },
  { key: 'Trung bình',icon: Shield,        color: 'text-yellow-600', bg: 'bg-yellow-500', border: 'border-yellow-400',  light: 'bg-yellow-50 border-yellow-200', desc: 'Lên lịch khám da liễu trong vòng 1–3 tháng.' },
  { key: 'Thấp',      icon: CheckCircle,   color: 'text-green-600',  bg: 'bg-green-500',  border: 'border-green-400',   light: 'bg-green-50 border-green-200',   desc: 'Khuyến nghị kiểm tra da định kỳ hàng năm. Theo dõi các dấu hiệu bất thường.' },
]

export default function RiskAssessmentCard({ prediction, knowledge }) {
  if (!prediction) return null

  const originalRisk = prediction.risk_level || 'Low'
  const riskMapping = {
    'Critical': 'Nguy Hiểm',
    'High': 'Cao',
    'Medium': 'Trung bình',
  ...
  const activeRisk = riskMapping[originalRisk] || 'Thấp'
  ...
  return (
    <div className="card space-y-4">
      ...
      <div className="flex items-center gap-2 pb-3 border-b border-gray-100">
        ...
        <div>
          <h3 className="text-sm font-bold text-gray-900">Đánh giá mức độ rủi ro</h3>
          <p className="text-xs text-gray-400">Phân loại mức độ khẩn cấp lâm sàng</p>
        </div>
      </div>
      ...
      <div className={`rounded-xl border px-4 py-4 flex items-center gap-4 ${active.light}`}>
        ...
        <div>
          <p className={`text-lg font-black ${active.color}`}>Mức độ {activeRisk}</p>
      ...
      <div className="space-y-2">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Thang đo rủi ro</p>
      ...
      {knowledge?.specialist_referral && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-start gap-2">
          ...
          <div>
            <p className="text-xs font-bold text-blue-700 mb-0.5">Chuyên gia đề xuất</p>
      ...
      {knowledge?.follow_up && (
        <div className="flex items-start gap-2 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
          ...
          <div>
            <p className="text-xs font-bold text-gray-600 mb-0.5">Lịch tái khám</p>
